...
from hash_model.models import Cart, User, OTP, Profile_Pic
...
async def registration(user: UserCreate = Body(default=None), db: Session = Depends(get_db)):
    ...
    if user.password == user.confirm_password:
        user = User(name=user.name, email=user.email, type=user.user_type,
        ...
        if not data:
            ...
            raise HTTPException(
                status_code=status.HTTP_208_ALREADY_REPORTED, detail="You are aleady exists")
    else:
        raise HTTPException(
...
async def image_upload_file(file: UploadFile = File(...),
    ...
    existed_data = db.query(Profile_Pic).filter(
        Profile_Pic.email == verified['sub']).first()
    if existed_data:
        raise HTTPException(status_code=status.HTTP_208_ALREADY_REPORTED,
                            detail="Already exists email please update your profile")
    user = db.query(User).filter(
        User.email == verified['sub'] and User.type == verified['type']).first()
    File_DIR = './static/profile_image/'
    profile_path = File_DIR + file.filename
    profile = Profile_Pic(name=user.name, email=user.email,
                          photo=profile_path, owner_id=user.id)
    db.add(profile)
    db.commit()
...
async def forgat_pass(user: ForgatPassword = Body(default=None), db: Session = Depends(get_db)):
    ...
    if user.email == data.email:
        ...
        user_entry = OTP(email=user.email, otp=otp,
        ...
        if user.email == otp_email.email:
            ...
            if num < 3:
                ...
                existing_data.update(
                    {"otp": otp, "status": False, "exp_time": time() + 60, "count_otp": num})
                db.commit()
                message = MessageSchema(
                    subject="MyApp Account Verification Email",
                    recipients=[user.email],  # List of Recipients
...
async def reset_pass(user: ResetPassword = Body(default=None), db: Session = Depends(get_db)):
    if user.token:
        verified = jwt.decode(user.token, setting.SECRET_KEY,
                              algorithms=setting.ALGORITHM)
        print(verified)
        if verified['expiry'] >= time():
            email: str = verified.get("sub")
...
async def curren_user(db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    ...
    if verified['expiry'] >= time():
        ...
        if not user:
            raise HTTPException(
                status_code=404, detail="Address not found of this user")
        print(user.address)
...
async def generate_pdf(
    ...
    if verified['expiry'] >= time():
        ...
        if not user:
            raise HTTPException(
                status_code=404, detail="Address not found of this user")
        file_dir = Jinja2Templates(directory='./static/view')
        if file_dir == None:
            os.mkdir(file_dir)
        template = file_dir.get_template('pdf.html')
        data = {"user": user}
...
@router.get("/getcart", tags=["User"])
async def fetch_cart_by_user(db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    if token == 'undefined':
        raise HTTPException(
    ...
    if verified['expiry'] >= time():
        ...
        if user.type == 'shop':
            raise HTTPException(
                status_code=404, detail="cart item not for you")
        cart_list = db.query(Cart).filter(Cart.user_id == user.id).all()
        return {"status": status.HTTP_200_OK, "data": cart_list}
    else:
        return {"status": "failed", "message": "You are not authorized"}
